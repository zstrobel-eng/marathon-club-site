...
const baseURL = process.env.BASE_URL || parseArg('url') || 'http://localhost:5000';
const headlessArg = parseArg('headless');
const headless = headlessArg === undefined ? true : headlessArg !== 'false' && headlessArg !== '0';
const navTimeout = parseInt(parseArg('timeout') || process.env.PUPPETEER_TIMEOUT || '30000', 10);
...
const pages = pagesArg ? pagesArg.split(',').map(p => p.trim()) : defaultPages;

async function runCheck(browser, path) {
	const page = await browser.newPage();
	const result = { path, url: null, status: null, ok: true, errors: [], warnings: [], loadTime: null };
	try {
		...
		const resp = await page.goto(url, { waitUntil: 'networkidle2', timeout: navTimeout });
...
function printLine(...parts) { console.log(parts.filter(Boolean).join(' ')); }

(async function main() {
	printLine('Puppeteer baseline runner ->', baseURL);
	printLine('Options:', `headless=${headless}`, `timeout=${navTimeout}ms`, `pages=${pages.length}`);
	...
	for (const r of results) {
		...
		printLine(state, r.path, r.status || '', r.loadTime ? `${r.loadTime}ms` : '');
